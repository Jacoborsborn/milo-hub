// supabase/functions/pt-meal-generator/index.ts
// Meal-only generator for PT Hub. No DB; no auth required. Uses static UK grocery rules.
...
Deno.serve(async (req) => {
  const requestId = crypto.randomUUID?.()?.slice(0, 8) ?? Date.now().toString(36);
  console.log(`[pt-meal-generator][${requestId}] Request: ${req.method} ${new URL(req.url).pathname}`);
  ...
  if (req.method !== "POST") {
    console.warn(`[pt-meal-generator][${requestId}] Rejected: method not allowed`);
  ...
  if (!OPENAI_API_KEY) {
    console.error(`[pt-meal-generator][${requestId}] Missing OPENAI_API_KEY`);
  ...
  if (FOODS_DATA_LEAN.length === 0) {
    console.error(`[pt-meal-generator][${requestId}] Food database not loaded (FOODS_DATA_LEAN.length=0)`);
  ...
  try {
    body = await req.json();
    console.log(`[pt-meal-generator][${requestId}] Body keys: ${Object.keys(body ?? {}).join(", ")}`);
  } catch (error) {
    console.error(`[pt-meal-generator][${requestId}] Invalid JSON body:`, error);
  ...
  if (!rawMealInputs) {
    console.warn(`[pt-meal-generator][${requestId}] Missing mealInputs in body`);
    return new Response(JSON.stringify({ error: "Must provide mealInputs" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  console.log(`[pt-meal-generator][${requestId}] mealInputs keys: ${Object.keys(rawMealInputs).join(", ")}`);

  // Apply defaults for mealInputs
  const mealInputs = {
    dietaryPreference: rawMealInputs.dietaryPreference ?? rawMealInputs.dietType ?? "balanced",
    caloriesTargetPerDay: rawMealInputs.caloriesTargetPerDay ?? rawMealInputs.calorieGoal ?? 2000,
    mealsPerDay: rawMealInputs.mealsPerDay ?? 4,
    days: rawMealInputs.days ?? rawMealInputs.daysPerWeek ?? 7,
    dietGoal: rawMealInputs.dietGoal ?? rawMealInputs.goal ?? "maintain",
    budgetTier: rawMealInputs.budgetTier ?? rawMealInputs.budget ?? "medium",
    allergies: rawMealInputs.allergies ?? rawMealInputs.avoid ?? [],
    restrictions: rawMealInputs.restrictions ?? [],
    preferredProteins: rawMealInputs.preferredProteins,
    ...rawMealInputs,
  };
  ...
  let mealPlan: any;
  let grocerySections: any[] = [];
  let groceryTotals: any = {};
  {
    try {
      const startTime = Date.now();
      console.log(`[pt-meal-generator][${requestId}] Pipeline starting at ${new Date().toISOString()}`);
          ...
          console.log(`[pt-meal-generator][${requestId}] Allergies: raw=${JSON.stringify(rawAllergies)} expanded=${JSON.stringify(allergies)}`);
          ...
          console.log(`[pt-meal-generator][${requestId}] Calling generateMealConcepts (diet=${diet}, days=${days}, mealsPerDay=${mealsPerDay})`);
          ...
          // PARSE: Extract structured meal data
              // ============================================================================
          console.log(`[pt-meal-generator][${requestId}] Parsing meal concepts (concepts length=${conceptsText?.length ?? 0})`);
          const parsedMeals = parseMealConcepts(conceptsText, days, mealsPerDay);
          console.log(`[pt-meal-generator][${requestId}] Parsed days: ${parsedMeals?.days?.length ?? 0}`);
          ...
          // ============================================================================
          console.log(`[pt-meal-generator][${requestId}] Calling generateRecipes`);
          const finalMealPlan = await generateRecipes(openai, cappedMealPlan, foodsMapWithOverrides);
          console.log(`[pt-meal-generator][${requestId}] Recipes generated, building grocery list`);
          ...
          // Output shape: mealPlan (structured by day), grocerySections, groceryTotals, notes
          mealPlan = {
      ...
      console.error(`[pt-meal-generator][${requestId}] ERROR:`, errMsg);
      console.error(`[pt-meal-generator][${requestId}] Stack:`, errStack);
      if (error?.cause) console.error(`[pt-meal-generator][${requestId}] Cause:`, error.cause);
      return new Response(
        JSON.stringify({
          success: false,
          error: errMsg,
          errorType: error?.name || "MealGenerationError",
        }),
  ...
  console.log(`[pt-meal-generator][${requestId}] Success: completed in ${totalTime}ms`);
