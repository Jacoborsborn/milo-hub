...
async function getPlanByIdUnsafe(planId: string): Promise<Plan | null> {
  ...
  console.log("[supabase share/meal/[token]/shopping getPlanByIdUnsafe] NEXT_PUBLIC_SUPABASE_URL:", url ?? "undefined");
...
async function getProfileForShare(ptUserId: string | undefined): Promise<{ brand_logo_url?: string | null } | null> {
  ...
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  console.log("[supabase share/meal/[token]/shopping getProfileForShare] NEXT_PUBLIC_SUPABASE_URL:", url ?? "undefined");
  if (!url || !key) return null;
  const supabase = createClient(url, key);
