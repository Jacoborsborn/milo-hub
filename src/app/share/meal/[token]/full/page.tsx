...
import PublicMealFullView from "./PublicMealFullView";
...
async function getPlanByIdUnsafe(planId: string): Promise<Plan | null> {
  ...
  console.log("[supabase share/meal/[token]/full getPlanByIdUnsafe] NEXT_PUBLIC_SUPABASE_URL:", url ?? "undefined");
...
async function getProfileForShare(ptUserId: string | undefined): Promise<ProfileForShare> {
  ...
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  console.log("[supabase share/meal/[token]/full getProfileForShare] NEXT_PUBLIC_SUPABASE_URL:", url ?? "undefined");
  if (!url || !key) return null;
  const supabase = createClient(url, key);
...
export default async function ShareMealFullPage({
  params,
}: {
  ...
  const weekCommencing = meta?.weekCommencing
    ...
    : new Date(plan.created_at).toLocaleDateString(undefined, { dateStyle: "long" });
